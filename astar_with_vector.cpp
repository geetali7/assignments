...
public:
    ...
    Node(int a, int b) : r(a), c(b), mat(a, vector<int>(b, -1)) {}
};

void printMatrix(const vector<vector<int>>& mat, int r, int c) {
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++)
            printf("%d ", mat[i][j]);
        printf("\n");
...
Node* newNode(const vector<vector<int>>& mat, int x, int y, int newX,
              int newY, int level, Node* parent, int r, int c) {
    Node* node = new Node(r, c);
    node->parent = parent;
    node->mat = mat; // Copying the entire matrix
...
int calculateCost(const vector<vector<int>>& initial, const vector<vector<int>>& final, int r, int c) {
    int count = 0;
    for (int i = 0; i < r; i++)
        for (int j = 0; j < c; j++)
...
void solve(const vector<vector<int>>& initial, int x, int y,
           const vector<vector<int>>& final, int r, int c) {
    ...
    while (!pq.empty()) {
        ...
        if (min->cost == 0) {
            cout << "Path from start to final state:\n";
            printPath(min, r, c);
            return;
        }
...
int main() {
    ...
    vector<vector<int>> start(r, vector<int>(c, 0));
    vector<vector<int>> final(r, vector<int>(c, 0));

    cout << "Enter start matrix:\n";
    for (int i = 0; i < r; i++)
        for (int j = 0; j < c; j++)
            cin >> start[i][j];
